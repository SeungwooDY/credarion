...
## Amendments (2026-07-04, applied when porting to main)

Validation against the confirmed double-counting investigation found the original
implementation carried the core emission bug into the new layer. Four gaps were
closed when this ADR landed on `main`:

1. **Duplicate-counterpart emission removed.** `_emit_group` (and the demoted
   aggregate fallback's `_emit_aggregate_matches`) no longer reuse row 0 of the
   smaller side for leftover rows in unequal groups. Leftovers are emitted with
   the other side `NULL` and zero deltas ("constituent" rows). Invariant, now
   tested: no `erp_record_id` / `statement_line_id` appears in two results.
2. **Group delta recorded exactly once.** Previously the group-level
   quantity/amount delta was stamped on every row result, so the dashboard's
   money-at-risk multiplied each aggregate discrepancy by the group size.
   Only the group's "primary" row carries the deltas + `discrepancy_type`;
   summing deltas over a run now equals the true delta.
3. **Symmetric `_split_by_balance` routing.** The old rule only routed
   statement-heavy (>1.4×) groups away from Layer 1, so the ERP-heavy direction
   (this ADR's own headline case — multi-day GRN rows vs one combined statement
   line) was consumed by L1's closest-quantity tiebreaker before Layer 3 ever
   saw it. Any count-imbalanced `(po, material)` group now routes to Layer 3,
   either direction, no threshold.
4. **Run-stats counting rules.** `matched_count` and `auto_match_rate` count
   DISTINCT matched statement lines (rate capped at 100% as a belt-and-braces
   invariant); `discrepancy_count` counts once per group (primary rows only).

`multi_po_dn.py` was deleted rather than orphaned (its review-bucket label is
retained for historical rows). The aggregate fallback's OR-gate/always-matched
semantics remain deferred as documented above.
