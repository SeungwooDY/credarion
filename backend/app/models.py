...
class Account(Base):
    """A paying customer (tenant). Owns one or more organizations (entities).

    `subscription_status` is the gate for login: only users belonging to an
    account whose status is active/trialing may sign in. The field is designed
    to be driven manually today and flipped by a Stripe webhook later.
    """
    ...
    # Optional pointer to the external billing record (e.g. Stripe customer id),
    # so a webhook can locate and update this account without schema changes.
...
class User(Base):
    """A login identity belonging to exactly one account."""
...
class Organization(Base):
    ...
    account: Mapped[Account | None] = relationship(back_populates="organizations")
