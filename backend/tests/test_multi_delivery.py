...
class TestMultiDeliveryAggregation:
    def test_multiple_erp_rows_match_combined_statement_line(self):
        ...
        assert {m.erp.erp_id for m in matches} == {1, 2, 3}
        assert 1 in {m.statement.line_id for m in matches}
    ...
    def test_aggregates_both_sides_when_statement_also_split(self):
        ...
        assert all(m.status == "matched" for m in matches)
        assert {m.erp.erp_id for m in matches} == {1, 2}
        assert {m.statement.line_id for m in matches} == {1, 2, 3}
        assert unmatched_erp == []
        assert unmatched_stmt == []
    ...
    def test_quantity_discrepancy_outside_tolerance(self):
        ...
        assert all(m.discrepancy_type == "quantity_over" for m in matches)
    ...
    def test_quantity_under_discrepancy(self):
        ...
        assert all(m.discrepancy_type == "quantity_under" for m in matches)
    ...
    def test_price_inconsistency_across_deliveries_not_aggregated(self):
        ...
        # No aggregated "matched" result — every ERP row flagged as a discrepancy
        assert matches
        assert all(m.status == "discrepancy" for m in matches)
        erp_results = [m for m in matches if m.erp.erp_id in {1, 2, 3}]
    ...
    def test_no_double_counting_across_groups(self):
        ...
        erp_ids = [m.erp.erp_id for m in matches]
        stmt_ids = [m.statement.line_id for m in matches]
    ...
    def test_amount_discrepancy_flagged_when_quantities_match(self):
        ...
        assert matches
        assert all(m.status == "discrepancy" for m in matches)
        assert all(m.discrepancy_type in ("price_higher", "price_lower") for m in matches)
    ...
    def test_price_inconsistency_with_multiple_statement_lines_no_data_loss(self):
        ...
        seen_stmt = {m.statement.line_id for m in matches} | {s.line_id for s in unmatched_stmt}
        seen_erp = {m.erp.erp_id for m in matches} | {e.erp_id for e in unmatched_erp}
